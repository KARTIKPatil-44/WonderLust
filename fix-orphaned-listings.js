...
const connectDB = async () => {
  try {
    await mongoose.connect(
      process.env.MONGODB_URL || "mongodb://localhost:27017/wanderlust"
    );
...
const fixOrphanedListings = async () => {
  try {
    ...
    console.log(
      `Updated ${updateResult.modifiedCount} listings with owner ${user.username}.`
    );
    ...
    if (remainingOrphaned.length === 0) {
      ...
      console.log(
        `⚠️  ${remainingOrphaned.length} listings still don't have owners.`
      );
    }
  } catch (error) {
